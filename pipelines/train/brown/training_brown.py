...
from utils.helper_functions import (load_fasttext_model,
...
def train_model(model, optimizer, initial_embeddings, similarity_graphs, syntactic_graphs, global_cooc_edge_index, global_cooc_edge_attr):
    """
    Train the ContextRGAT model.
...
def main():
    ...
    model = ContextRGAT(300, 256, 300)
    torch.save(model, os.path.join(MODELS_DIR, "brown", "brown_model_base.pth"))
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
    torch.save(optimizer, os.path.join(MODELS_DIR, "brown", "brown_optimizer_base.pth"))

    model.train()
    train_model(model, optimizer, initial_embeddings, similarity_graphs, syntactic_graphs, global_cooc_edge_index, global_cooc_edge_attr)
